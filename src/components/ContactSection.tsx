...
const ContactSection = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const phoneNumber = "9342549643";
    const text = `Name: ${form.name}\nEmail: ${form.email}\nMessage: ${form.message}`;
    const encodedText = encodeURIComponent(text);
    window.open(`https://wa.me/91${phoneNumber}?text=${encodedText}`, "_blank");
  };

  const copyEmail = () => {
    navigator.clipboard.writeText("nancymargaret13@gmail.com");
    setCopied(true);
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4 p-4 border border-foreground/10 bg-white/50 hover:border-black transition-colors duration-300">
              ...
              <div>
                ...
                <p className="font-mono text-sm">+91 9342549643</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-4">
              {[
                {
                  Icon: Linkedin,
                  href: "https://linkedin.com/in/nancymargaret",
                },
              ].map(({ Icon, href }, i) => (
                <a
                  key={i}
